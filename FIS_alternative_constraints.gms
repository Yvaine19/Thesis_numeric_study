...
zz(i,j,c) "auxiliary variable to determine Z";
...
maint_1(i,j,m,c)$edg(i,j).. x(j,c) + x(i,c) + xx(m,c) =l= 2 + zz(i,j,c);
maint_2(i,j,c)$edg(i,j).. x(j,c) + x(i,c) =g= 2*zz(i,j,c);
maint_3(i,j,m,c)$edg(i,j).. x(i,c) + xx(m,c) =g= 2*zz(i,j,c);
z_sum(i,j)$edg(i,j).. z(i,j) =e= sum(c, zz(i,j,c)) 
